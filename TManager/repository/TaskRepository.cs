...
=======
        public virtual Task GetById(string taskId)
>>>>>>> 0e502141f256c07c2be5b5ff8b667f906c62e2c9
        {
            using (SQLiteConnection connection = new SQLiteConnection(CONNECTION_STRING))
            {
...
<<<<<<< HEAD
                ...
                return count > 0;
=======
                cmd.CommandText = string.Format("SELECT * FROM tasks LEFT JOIN users ON tasks.user_id = users.id WHERE tasks.id = {0}", taskId);
                using (SQLiteDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
